...
export default function ThemeToggle({ mode, toggleMode }: ThemeToggleProps) {
  ...
  return (
    ...
    >
      <Moon
        size={16}
        color={TOGGLE_COLORS.dark.icon}
        style={{ zIndex: 1, opacity: isDark ? 1 : 0, transition: "opacity 0.3s" }}
      />
      <Sun
        size={16}
        color={TOGGLE_COLORS.light.icon}
        style={{ zIndex: 1, opacity: isDark ? 0 : 1, transition: "opacity 0.3s" }}
      />
